...
for i in results:
    ...
    if (i[0]):
        ...
        if (i[3].isnumeric()):
            lexemes.append(["NUMBR Literal",i[3]])
        elif (is_integer(i[3])):
            lexemes.append(["signed NUMBR Literal",i[3]])
        elif (is_float(i[3])):
            lexemes.append(["NUMBAR Literal",i[3]])
        elif (i[3] == "WIN" or i[3] == "FAIL"):
            lexemes.append(["TROOF Literal",i[3]])
    elif (i[4]):
        if (i[4] in declaredIdentifiers):
            index = declaredIdentifiers.index(i[4])
            lexemes.append([declaredIdentifiersType[index],i[4]])
        else:
            if (len(lexemes) != 0):
                previousLexeme = lexemes[len(lexemes)-1][1]
                declaredIdentifiers.append(i[4])
                if (previousLexeme == "I HAS A"):       
                    lexemes.append(["Variable Identifier",i[4]])
                    declaredIdentifiersType.append("Variable Identifier")
                #one more for loop identifier (also func identifier)
                else:
                    lexemes.append(["IDENTIFIER",i[4]])
